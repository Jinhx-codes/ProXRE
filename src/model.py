...
class Encoder(nn.Module):
    ...
    def forward(self,
        ...
        for batch_i in range(batch_size):
            ...
            class_scores_2 = torch.sum(class_scores_2,dim=0,keepdim=True)
            class_scores = class_scores_1 - class_scores_2  # [base_nota_num, nota_in_episode]
            alpha = F.softmax(class_scores)
            correction_nota_embeddings = torch.matmul(alpha,episode_nota_embeddings)
        ...
        for batch_i in range(batch_size):
            ...
            for query_i in range(num_queries):
                ...
                hs = torch.index_select(entity_embs, 0, ht_i[:, 0])  # [pair, hidden] [420,768]
                ts = torch.index_select(entity_embs, 0, ht_i[:, 1])  # [pair, hidden]

                h_att = torch.index_select(entity_atts, 0, ht_i[:, 0])  # [pair, head, max_len] [420,12,409]
                t_att = torch.index_select(entity_atts, 0, ht_i[:, 1])  # [pair, head, max_len]
                ht_att = F.normalize((h_att * t_att).mean(1), p=1, dim=1)  # [pair, max_len]  [420,409]
                ...
                for class_prototypes in prototypes[batch_i]:
                    ...
                    if self.distance_metric == "dot_product" and nota_flag:
                        ...
                        class_scores = -torch.sum(class_scores, dim=-1)  # [1 or base_nota_num, pair]
                    class_scores = class_scores.max(dim=0, keepdim=False)[0]  # [pair]  [420]
                    scores.append(class_scores)
                scores = torch.stack(scores).transpose(0, 1)  # [pair, r_types_in_an_episode]   [420,5] r_types_in_an_episode
                predictions_binary = self.loss_fnt.get_label(scores.detach(),
                                                             num_labels=num_classes)  # [pair, r_types_in_an_episode]   [420,5]
